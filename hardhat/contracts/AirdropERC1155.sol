...
import "./deps/IAirdropERC1155.sol";
    ...
    IAirdropERC1155
{
     ...
     */
    function airdropERC1155(address _tokenAddress, address _tokenOwner, AirdropContent[] calldata _contents)
        external
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < len;) {
            try IERC1155(_tokenAddress).safeTransferFrom(
                _tokenOwner, _contents[i].recipient, _contents[i].tokenId, _contents[i].amount, ""
            ) {} catch {
                ...
                emit AirdropFailed(
                    _tokenAddress, _tokenOwner, _contents[i].recipient, _contents[i].tokenId, _contents[i].amount
                );
            }
